...
{
    ap_log_cerror(APLOG_MARK, APLOG_DEBUG, 0, c, APLOGNO(4200)
            "ap_process_http_connection");
    if (async_mpm && !c->clogging_input_filters) {
        ap_log_cerror(APLOG_MARK, APLOG_DEBUG, 0, c, APLOGNO(4201)
                "ap_process_http_async_connection");
        return ap_process_http_async_connection(c);
    }
    else {
        ap_log_cerror(APLOG_MARK, APLOG_DEBUG, 0, c, APLOGNO(4202)
